...
export default function VideoThumb({
  ...
  if (active) {
    return (
      <div
        ...
        className={`relative overflow-hidden rounded-card border border-on-dark/10 bg-ink-800 ${ratioClass}`}
      >
        <iframe
          ref={iframeRef}
  ...
  return (
    <button
      ...
      className={`group relative block w-full overflow-hidden rounded-card border border-on-dark/10 bg-ink-800 ${ratioClass}`}
    >
      <Image
        src={thumbSrc}
        alt={title}
      ...
      <div className="absolute inset-0 grid place-items-center">
        <span className="grid h-14 w-14 place-items-center rounded-full bg-accent-primary text-on-dark shadow-glow transition duration-300 group-hover:scale-110">
          <Play size={20} fill="currentColor" />
        </span>
      </div>
      {title ? (
        <div className="absolute inset-x-0 bottom-0 p-4 text-left">
          <p className="line-clamp-2 text-sm font-medium text-on-dark drop-shadow">{title}</p>
        </div>
      ) : null}
    </button>
  );
