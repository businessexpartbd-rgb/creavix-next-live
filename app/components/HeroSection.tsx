...
function ThemeRig({ theme }: { theme: HeroTheme }) {
  switch (theme) {
    case 'home':
      return (
        <div className="pointer-events-none absolute inset-0 -z-[1] hidden md:block">
          ...
          />
          <Play
            className="absolute right-[12%] top-[58%] h-12 w-12 animate-pulse-glow text-brand/35"
            fill="currentColor"
          />
        </div>
      );
    ...
    case 'portfolio':
      return (
        <div className="pointer-events-none absolute inset-0 -z-[1] hidden md:block">
          <Grid3x3 className="absolute right-[6%] top-[24%] h-20 w-20 animate-float text-brand/30" />
          <Play
            className="absolute right-[20%] top-[44%] h-12 w-12 animate-pulse-glow text-brand/40"
            fill="currentColor"
          />
          <Film
    ...
    case 'about':
      return (
        <div className="pointer-events-none absolute inset-0 -z-[1] hidden md:block">
          <MapPin className="absolute right-[6%] top-[22%] h-14 w-14 animate-float text-brand/40" />
          <span className="absolute right-[18%] top-[48%] flex gap-2">
            {[0, 1, 2].map((i) => (
              <span
                key={i}
                className="h-3 w-3 animate-pulse rounded-full bg-brand/50"
    ...
    case 'pricing':
      return (
        <div className="pointer-events-none absolute inset-0 -z-[1] hidden md:block">
          <Tag className="absolute right-[6%] top-[22%] h-14 w-14 animate-float text-brand/40" />
