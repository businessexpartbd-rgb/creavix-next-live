    ...
    <div className="space-y-3">
      {items.map((item, i) => {
        ...
        return (
          <div
            key={item.q_en}
            className="overflow-hidden rounded-card border border-black/10 bg-ink-800"
          >
            <button
              type="button"
            ...
            >
              <span className="flex flex-col">
                <span className="font-display text-lg uppercase tracking-[0.04em] text-ash-50">
                  {item.q_en}
                </span>
                <span className="font-bn text-sm text-ash-300">{item.q_bn}</span>
              </span>
              <span
                className={`grid h-9 w-9 flex-none place-items-center rounded-full border border-black/10 bg-white/5 text-ash-300 transition ${
